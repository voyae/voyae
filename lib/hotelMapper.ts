...
export function mapHotels(hotels: any[]): HotelCard[] {
    ...
    return hotels.map((hotel: any) => {
        ...
        let firstImg = Array.isArray(targetImageSource) ? targetImageSource[0] : null;

        if (typeof firstImg === 'object' && firstImg !== null) {
            firstImg = firstImg.url ?? firstImg.src ?? firstImg.highResUrl ?? firstImg.large ?? "";
        }

        let rawImg = 
            (typeof firstImg === 'string' && firstImg.startsWith('http')) ? firstImg : (
                hotel.main_photo ??
                hotel.thumbnail ??
                hotel.image ??
                hotel.photo ??
        ...
        if (typeof rawImg === 'object' && rawImg !== null) {
            rawImg = (rawImg as any).url ?? (rawImg as any).src ?? "";
        }
